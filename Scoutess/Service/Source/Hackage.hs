...
import Scoutess.Core                         (viName, viVersion, createVersionInfo, findCabalFiles)
import Scoutess.Types  --                        (VersionInfo(..), VersionSpec(..))
...
fetchVersionsHackage sourceConfig = liftIO $ do
  createDirectoryIfMissing True (srcCacheDir sourceConfig)
  pkgIndex' <- updateFile "http://hackage.haskell.org/packages/archive/00-index.tar.gz" (srcCacheDir sourceConfig </> "00-index.tar.gz")
  case pkgIndex' of
    Nothing       -> return . Left . SourceErrorOther $ "Couldn't download the package index."
    Just pkgIndex -> do
      extractArchive pkgIndex (tmpDir </> "unpack")
      let prefFile = tmpDir </> "unpack" </> "preferred-versions"
      prefExists <- doesFileExist prefFile
      pref <- if prefExists then (Just . Text.pack) `liftM` (readFile prefFile) else return Nothing
      cabals              <- findCabalFiles tmpDir
      versionInfos        <- forM cabals $ \cabalFile -> do
